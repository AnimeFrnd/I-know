...
async def delete_files(messages, client, k, enter):
    ...
    await k.edit_text(
        "<blockquote><b>ᴀʟʟ ᴛʜᴇ ғɪʟᴇ(s) ʜᴀᴠᴇ ʙᴇᴇɴ ᴅᴇʟᴇᴛᴇᴅ ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs 📂</b></blockquote>",
...
def force_sub(func):
    ...
    async def wrapper(client, message):
        SPOILER = get_spoiler()
        msg = await message.reply_photo(caption="<code>Connecting!</code>", photo="https://envs.sh/9Nk.jpg", has_spoiler=SPOILER)
